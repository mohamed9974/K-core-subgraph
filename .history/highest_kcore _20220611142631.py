...
def network_init(file_name):
    '''
    :param file_name: the name of the file that contains the network
    :return: a networkx graph object
    '''
    network = nx.read_edgelist(file_name, delimiter='\t')
    return network

def get_k_core_list(network, k):
    '''
    :param network: a networkx graph object
    :param k: the k-core
    :return: a list of the nodes in the k-core
    '''
    k_core_list = list(nx.k_core(network, k).nodes())
    return k_core_list

def get_highest_k_core(network, k):
    '''
    :param network: a networkx graph object
    :param k: the k-core
    :return: the number of proteins in the k-core
    '''
    k_core_list = get_k_core_list(network, k)
    return len(k_core_list)

def main():
    '''
    :return:
    '''
    network = network_init('./BioGRID.txt')
    print(network['A1BG'])
    print(get_highest_k_core(network, 4))
    #loop over all the k-cores and print the number of proteins in each k-core
    for k in range(1,len(network.nodes())+1):
        print('For k =', k, 'there are', get_highest_k_core(network, k), 'proteins.')
